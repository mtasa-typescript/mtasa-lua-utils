...
function copyFiles() {
    ...
    fg.sync(escapeGlob(rootDir) + '/**', { ignore: '**/*.ts' }).forEach(
        resourceFile => {
            ...
            console.log(
                `[INFO] File "${resourceFile}" copied to "${outputFile}"`,
            );
        },
    );
    ...
    fg.sync('vendor/**').forEach(resourceFile => {
        fg.sync(escapeGlob(outDir) + '/*', {
            onlyFiles: false,
            onlyDirectories: true,
        }).forEach(outputPath => {
            const outputFile = resourceFile.replace(
                'vendor/',
                outputPath + '/',
            );
            ensureDirExists(outputFile);
    ...
    return resourcesMetaPath
        .map(resourceMetaPath => {
            const resourceDirName = path
                .dirname(resourceMetaPath)
                .split('/')
                .pop();
            if (
                resourceDirName.startsWith('[') &&
                resourceDirName.endsWith(']')
            )
                return undefined;
...
function checkResource(resourcePath) {
    ...
    fg.sync(escapeGlob(resourcePath) + '/**', {
        ignore: '**/meta.xml',
    }).forEach(resourceFile => {
        if (metaString.indexOf(path.parse(resourceFile).base) === -1)
            console.warn(
                `[WARN] File "${
                    path.parse(resourceFile).base
                }" does not exists in meta.xml`,
            );
    });
}
...
getResourcesPath().forEach(resourcePath => {
    ...
    console.log(
        `[INFO] Resource compiled at "${path.join(outDir, resourcePath)}".`,
    );
});
