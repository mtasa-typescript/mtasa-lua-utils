 ...
 */
function removeFunctionExportModifier(
    node: ts.Node,
 ...
 */
function removeClassExportModifier(
    node: ts.Node,
    ctx: ts.TransformationContext,
): ResultType {
 ...
 */
function removeVariableExportModifier(
    node: ts.Node,
    ctx: ts.TransformationContext,
): ResultType {
    ...
    for (const name of names) {
        globalExpressions.push(
            generateGlobalExpression(name, ctx.factory, true),
        );
    }
    return [node, ...globalExpressions];
}

export default function removeExportModifierIfPossible(
    node: ts.Node,
    ctx: ts.TransformationContext,
): ResultType {
